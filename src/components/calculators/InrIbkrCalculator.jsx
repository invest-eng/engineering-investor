...
export default function InrIbkrCalculator() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #020617 0%, #0f172a 50%, #1e293b 100%)', padding: '1.5rem', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>

        {/* Header */}
        <div style={{ marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: 'linear-gradient(135deg, #10b981, #065f46)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <TrendingUp size={28} color="white" />
            </div>
            <div>
              <h1 style={{ fontSize: '2rem', fontWeight: 800, color: 'white', margin: 0 }}>INR <span style={{ color: '#34d399' }}>vs</span> IBKR</h1>
              <p style={{ color: '#94a3b8', fontSize: '0.8rem', margin: 0 }}>Uradni cenik Ilirike (5.3.2026) + FIFO + validacija INR limitov</p>
            </div>
          </div>
        </div>
        ...
        {validation.errors.length > 0 && (
          <div style={{ marginBottom: '1rem', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.4)', borderRadius: '12px', padding: '1rem' }}>
            ...
            {validation.errors.map((e, i) => <p key={i} style={{ color: '#fca5a5', fontSize: '0.75rem', margin: '2px 0' }}>{e}</p>)}
          </div>
        )}

        {/* Warnings */}
        {validation.warnings.length > 0 && (
          <div style={{ marginBottom: '1rem', background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.4)', borderRadius: '12px', padding: '1rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#fbbf24', fontWeight: 600, fontSize: '0.875rem', marginBottom: '0.5rem' }}>
              <AlertTriangle size={18} /> Opozorila
            </div>
            {validation.warnings.map((w, i) => <p key={i} style={{ color: '#fde68a', fontSize: '0.75rem', margin: '2px 0' }}>{w}</p>)}
          </div>
        )}
        ...
        <div style={{ marginBottom: '1rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          <button onClick={() => setShowEducation(!showEducation)}
            style={{ padding: '0.5rem 1rem', background: 'rgba(245,158,11,0.15)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: '8px', color: '#fcd34d', fontSize: '0.875rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <BookOpen size={16} />{showEducation ? 'Skrij' : 'Razlaga'} davkov
          </button>
        </div>

        {showEducation && (
          <div style={{ marginBottom: '1.5rem', background: 'rgba(30,41,59,0.8)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: '12px', padding: '1.25rem' }}>
            <h3 style={{ color: 'white', fontWeight: 700, marginTop: 0 }}>Davki in stroški</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem' }}>
              <div style={{ background: 'rgba(15,23,42,0.5)', borderRadius: '8px', padding: '1rem' }}>
                <h4 style={{ color: '#34d399', marginTop: 0 }}>INR stroški</h4>
                <ul style={{ color: '#cbd5e1', fontSize: '0.75rem', paddingLeft: '1rem' }}>
              ...
              <div style={{ background: 'rgba(15,23,42,0.5)', borderRadius: '8px', padding: '1rem' }}>
                <h4 style={{ color: '#60a5fa', marginTop: 0 }}>IBKR stroški</h4>
                <ul style={{ color: '#cbd5e1', fontSize: '0.75rem', paddingLeft: '1rem' }}>
            ...
            <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: '8px', padding: '0.75rem', marginTop: '0.75rem' }}>
              <p style={{ color: '#fde68a', fontSize: '0.75rem', margin: 0 }}><strong>INR limiti:</strong> Prvo leto max €20.000, nato €5.000/leto.</p>
            </div>
          </div>
        )}

        {/* Scenarios */}
        <div style={{ marginBottom: '1.5rem', background: 'rgba(30,41,59,0.5)', border: '1px solid rgba(71,85,105,0.5)', borderRadius: '12px', padding: '1rem' }}>
          <p style={{ color: '#e2e8f0', fontSize: '0.875rem', fontWeight: 600, marginTop: 0, marginBottom: '0.75rem' }}>Scenariji</p>
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            {Object.entries(SCENARIOS).map(([key, s]) => (
              <button key={key} onClick={() => loadScenario(key)}
                style={{ padding: '0.4rem 0.875rem', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 500, cursor: 'pointer', border: 'none', background: activeScenario === key ? '#10b981' : '#334155', color: activeScenario === key ? 'white' : '#cbd5e1', transition: 'all 0.15s' }}>
                {s.name}
              </button>
            ))}
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr)', gap: '1.5rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              {/* Osnovni parametri */}
              <div style={{ background: 'rgba(30,41,59,0.5)', border: '1px solid rgba(71,85,105,0.5)', borderRadius: '12px', padding: '1.25rem' }}>
                <h3 style={{ color: 'white', fontSize: '0.875rem', fontWeight: 600, marginTop: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                {[
                  ...
                  <div key={p.label} style={{ marginBottom: '1rem' }}>
                    <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#cbd5e1', marginBottom: '0.35rem' }}>
                      {p.label}: <span style={{ color: '#34d399' }}>{p.display}</span>
                    </label>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#cbd5e1', marginBottom: '0.35rem' }}>
                    Začetni kapital: <span style={{ color: initialCapital > 20000 ? '#f87171' : '#34d399' }}>€{initialCapital.toLocaleString()}</span>
                    {initialCapital > 20000 && <span style={{ color: '#f87171', fontSize: '0.65rem', marginLeft: '0.5rem' }}>nad INR limitom</span>}
                  </label>
                  <input type="range" min={0} max={100000} step={1000} value={initialCapital} onChange={e => setInitialCapital(Number(e.target.value))} style={{ width: '100%', accentColor: '#10b981' }} />
                </div>

                {/* Način vlaganja */}
                <div style={{ borderTop: '1px solid #334155', paddingTop: '1rem' }}>
                  <p style={{ color: '#cbd5e1', fontSize: '0.75rem', fontWeight: 600, marginBottom: '0.5rem' }}>Način vlaganja</p>
                  {[
                    ...
                    <label key={opt.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', cursor: 'pointer', padding: '0.4rem', borderRadius: '6px', marginBottom: '0.25rem' }}>
                      <input type="radio" name="investMode" value={opt.id} checked={investmentMode === opt.id} onChange={e => setInvestmentMode(e.target.value)} style={{ marginTop: '2px', accentColor: '#10b981' }} />
                      <div>
                        <p style={{ color: '#e2e8f0', fontSize: '0.8rem', fontWeight: 500, margin: 0 }}>{opt.label}</p>
                        <p style={{ color: '#64748b', fontSize: '0.7rem', margin: 0 }}>{opt.desc}</p>
                      </div>
                    </label>
                  ))}
                </div>
                ...
                {investmentMode === 'dca' && (
                  <div style={{ marginTop: '0.75rem' }}>
                    <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#cbd5e1', marginBottom: '0.35rem' }}>
                      Mesečno: <span style={{ color: monthlyContribution * 12 > 5000 ? '#fbbf24' : '#34d399' }}>€{monthlyContribution}</span>
                      {monthlyContribution * 12 > 5000 && <span style={{ color: '#fbbf24', fontSize: '0.65rem', marginLeft: '0.5rem' }}>€{(monthlyContribution * 12).toLocaleString()}/leto nad INR limitom</span>}
                    </label>
                    <input type="range" min={0} max={2000} step={50} value={monthlyContribution} onChange={e => setMonthlyContribution(Number(e.target.value))} style={{ width: '100%', accentColor: '#10b981' }} />
                  </div>
                )}
                ...
                {investmentMode === 'annual' && (
                  <div style={{ marginTop: '0.75rem' }}>
                    <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#cbd5e1', marginBottom: '0.35rem' }}>
                      Letni vložek: <span style={{ color: annualLumpSum > 5000 ? '#fbbf24' : '#34d399' }}>€{annualLumpSum.toLocaleString()}</span>
                      {annualLumpSum > 5000 && <span style={{ color: '#fbbf24', fontSize: '0.65rem', marginLeft: '0.5rem' }}>nad INR limitom €5k/leto</span>}
                    </label>
                    <input type="range" min={1000} max={50000} step={1000} value={annualLumpSum} onChange={e => setAnnualLumpSum(Number(e.target.value))} style={{ width: '100%', accentColor: '#10b981' }} />
                  </div>
                )}

                <div style={{ borderTop: '1px solid #334155', paddingTop: '0.75rem', marginTop: '0.75rem' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                    <input type="checkbox" checked={openInr2026} onChange={e => setOpenInr2026(e.target.checked)} style={{ accentColor: '#10b981' }} />
                    <span style={{ color: '#e2e8f0', fontSize: '0.8rem' }}>50% popust (odprtje INR v 2026)</span>
                  </label>
                </div>
              </div>
              ...
              <div style={{ background: 'rgba(30,41,59,0.5)', border: '1px solid rgba(71,85,105,0.5)', borderRadius: '12px', padding: '1.25rem' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                  <input type="checkbox" checked={customIbkr} onChange={e => setCustomIbkr(e.target.checked)} style={{ accentColor: '#3b82f6' }} />
                  <span style={{ color: 'white', fontSize: '0.875rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                    <Settings2 size={16} color="#60a5fa" /> Ločeni IBKR parametri
                  </span>
                </label>
                {customIbkr && (
                  <div style={{ marginTop: '0.75rem', paddingTop: '0.75rem', borderTop: '1px solid #334155' }}>
                    <p style={{ color: '#64748b', fontSize: '0.7rem', marginTop: 0 }}>IBKR nima limitov.</p>
                    <div style={{ marginBottom: '0.75rem' }}>
                      <label style={{ display: 'block', fontSize: '0.75rem', color: '#93c5fd', marginBottom: '0.25rem' }}>IBKR začetni: <span style={{ color: '#60a5fa' }}>€{ibkrInitialCapital.toLocaleString()}</span></label>
                      <input type="range" min={0} max={200000} step={1000} value={ibkrInitialCapital} onChange={e => setIbkrInitialCapital(Number(e.target.value))} style={{ width: '100%', accentColor: '#3b82f6' }} />
                    </div>
                    {investmentMode === 'dca' && (
                      <div>
                        <label style={{ display: 'block', fontSize: '0.75rem', color: '#93c5fd', marginBottom: '0.25rem' }}>IBKR mesečno: <span style={{ color: '#60a5fa' }}>€{ibkrMonthlyContribution}</span></label>
                        <input type="range" min={0} max={5000} step={50} value={ibkrMonthlyContribution} onChange={e => setIbkrMonthlyContribution(Number(e.target.value))} style={{ width: '100%', accentColor: '#3b82f6' }} />
                      </div>
                    )}
                    {investmentMode === 'annual' && (
                      <div>
                        <label style={{ display: 'block', fontSize: '0.75rem', color: '#93c5fd', marginBottom: '0.25rem' }}>IBKR letno: <span style={{ color: '#60a5fa' }}>€{ibkrAnnualLumpSum.toLocaleString()}</span></label>
                        <input type="range" min={1000} max={100000} step={1000} value={ibkrAnnualLumpSum} onChange={e => setIbkrAnnualLumpSum(Number(e.target.value))} style={{ width: '100%', accentColor: '#3b82f6' }} />
                      </div>
                    )}
                  </div>
                )}
              ...
              {/* ETF */}
              <div style={{ background: 'rgba(30,41,59,0.5)', border: '1px solid rgba(71,85,105,0.5)', borderRadius: '12px', padding: '1.25rem' }}>
                <h3 style={{ color: 'white', fontSize: '0.875rem', fontWeight: 600, marginTop: 0 }}>ETF</h3>
                <select value={selectedEtf} onChange={e => setSelectedEtf(e.target.value)}
                  style={{ width: '100%', padding: '0.5rem 0.75rem', borderRadius: '8px', background: '#334155', color: 'white', border: '1px solid #475569', fontSize: '0.875rem', marginBottom: '0.75rem' }}>
                  {Object.entries(ETF_PROFILES).map(([key, p]) => <option key={key} value={key}>{p.name} - {p.fullName}</option>)}
                </select>
                <div style={{ background: 'rgba(15,23,42,0.5)', borderRadius: '8px', padding: '0.75rem', fontSize: '0.75rem' }}>
                  <p style={{ color: 'white', fontWeight: 600, margin: '0 0 0.25rem' }}>{etf.fullName}</p>
                  <p style={{ color: '#94a3b8', margin: '0 0 0.5rem' }}>{etf.description}</p>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                    <div><p style={{ color: '#64748b', fontSize: '0.65rem', margin: 0 }}>DIV YIELD</p><p style={{ color: '#34d399', fontWeight: 700, margin: 0 }}>{etf.grossDividendYield}%</p></div>
                    <div><p style={{ color: '#64748b', fontSize: '0.65rem', margin: 0 }}>TIP</p><p style={{ color: '#cbd5e1', margin: 0 }}>{etf.distribution}</p></div>
                    <div><p style={{ color: '#64748b', fontSize: '0.65rem', margin: 0 }}>TER</p><p style={{ color: '#cbd5e1', margin: 0 }}>{etf.expense}%</p></div>
                    <div><p style={{ color: '#64748b', fontSize: '0.65rem', margin: 0 }}>DONOS</p><p style={{ color: '#60a5fa', fontWeight: 700, margin: 0 }}>{etf.historicalReturn}%</p></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              {/* Tabs */}
              <div style={{ display: 'flex', gap: '4px', background: 'rgba(30,41,59,0.4)', padding: '4px', borderRadius: '10px', overflowX: 'auto' }}>
                {[{ id: 'summary', label: 'Povzetek' }, { id: 'breakdown', label: 'Stroški' }, { id: 'fifo', label: 'FIFO' }, { id: 'transparency', label: 'Izračun' }].map(tab => (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                    style={{ flex: 1, padding: '0.5rem 0.75rem', borderRadius: '7px', fontSize: '0.8rem', fontWeight: 500, cursor: 'pointer', border: 'none', whiteSpace: 'nowrap', background: activeTab === tab.id ? '#334155' : 'transparent', color: activeTab === tab.id ? 'white' : '#94a3b8', transition: 'all 0.15s' }}>
                    {tab.label}
                  </button>
                ))}
              </div>
              ...
              {activeTab === 'summary' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                    <div style={{ background: calculations.inrBlocked ? 'rgba(239,68,68,0.1)' : 'rgba(16,185,129,0.1)', border: `1px solid ${calculations.inrBlocked ? 'rgba(239,68,68,0.4)' : 'rgba(16,185,129,0.4)'}`, borderRadius: '12px', padding: '1.25rem' }}>
                      {calculations.inrBlocked ? (
                        <>
                          <p style={{ color: '#fca5a5', fontSize: '0.75rem', fontWeight: 600, margin: '0 0 0.25rem' }}>INR BLOKIRAN</p>
                          <p style={{ color: '#f87171', fontSize: '1.5rem', fontWeight: 700, margin: '0 0 0.5rem' }}>Nemogoče</p>
                          <p style={{ color: '#fca5a5', fontSize: '0.7rem', margin: 0 }}>Prilagodi parametre na INR omejitve.</p>
                        </>
                      ) : (
                        <>
                          <p style={{ color: '#6ee7b7', fontSize: '0.7rem', fontWeight: 600, margin: '0 0 0.25rem' }}>INR KONČNA VREDNOST</p>
                          <p style={{ color: '#34d399', fontSize: '2rem', fontWeight: 800, margin: '0 0 0.75rem' }}>€{calculations.inrFinalValue.toLocaleString()}</p>
                    ...
                    <div style={{ background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.4)', borderRadius: '12px', padding: '1.25rem' }}>
                      <p style={{ color: '#93c5fd', fontSize: '0.7rem', fontWeight: 600, margin: '0 0 0.25rem' }}>IBKR KONČNA VREDNOST</p>
                      <p style={{ color: '#60a5fa', fontSize: '2rem', fontWeight: 800, margin: '0 0 0.75rem' }}>€{calculations.ibkrFinalValue.toLocaleString()}</p>
                  ...
                  {!calculations.inrBlocked && (
                    <div style={{ background: advantage > 0 ? 'rgba(16,185,129,0.15)' : 'rgba(59,130,246,0.15)', border: `1px solid ${advantage > 0 ? 'rgba(16,185,129,0.5)' : 'rgba(59,130,246,0.5)'}`, borderRadius: '12px', padding: '1.5rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.5rem' }}>
                        <p style={{ color: '#cbd5e1', fontSize: '0.875rem', fontWeight: 600, margin: 0 }}>Prednost</p>
                        <span style={{ fontSize: '1.25rem', fontWeight: 700, color: advantage > 0 ? '#34d399' : '#60a5fa' }}>{advantage > 0 ? 'INR' : 'IBKR'}</span>
                      </div>
                      <p style={{ fontSize: '2.5rem', fontWeight: 800, color: advantage > 0 ? '#34d399' : '#60a5fa', margin: '0 0 0.25rem' }}>
                      ...
                      {customIbkr && calculations.totalInvestedInr !== calculations.totalInvestedIbkr && (
                        <p style={{ color: '#fbbf24', fontSize: '0.7rem', margin: '0.5rem 0 0' }}>Primerjava ni poštena: različni vložki.</p>
                      )}
                    </div>
                  )}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.5rem' }}>
                    {[
                      { label: 'INR vloženo', value: `€${calculations.totalInvestedInr.toLocaleString()}`, color: 'white' },
                      { label: 'IBKR vloženo', value: `€${calculations.totalInvestedIbkr.toLocaleString()}`, color: 'white' },
                      { label: 'IBKR davek', value: `${calculations.effectiveIbkrTaxRate}%`, color: '#60a5fa' },
                      { label: 'INR davek', value: `${(calculations.inrTaxRate * 100).toFixed(0)}%`, color: '#34d399' },
                    ].map(s => (
                      <div key={s.label} style={{ background: 'rgba(30,41,59,0.5)', border: '1px solid rgba(71,85,105,0.5)', borderRadius: '8px', padding: '0.75rem' }}>
                        <p style={{ color: '#64748b', fontSize: '0.65rem', fontWeight: 600, textTransform: 'uppercase', margin: '0 0 0.25rem' }}>{s.label}</p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {!calculations.inrBlocked && (
                    <div style={{ background: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: '12px', padding: '1.25rem' }}>
                      <h4 style={{ color: '#34d399', marginTop: 0 }}>INR (Ilirika)</h4>
                      {[
                        ...
                        <div key={i.l} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: '#cbd5e1', marginBottom: '0.25rem' }}>
                          <span>{i.l}</span><span style={{ color: '#f87171' }}>-€{Math.round(i.v).toLocaleString()}</span>
                        </div>
                      ))}
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', fontWeight: 700, color: 'white', borderTop: '1px solid rgba(16,185,129,0.3)', paddingTop: '0.5rem', marginTop: '0.5rem' }}>
                        <span>SKUPAJ:</span><span style={{ color: '#f87171' }}>-€{calculations.inrTotalCosts.toLocaleString()}</span>
                      </div>
                    </div>
                  )}
                  <div style={{ background: 'rgba(59,130,246,0.05)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: '12px', padding: '1.25rem' }}>
                    <h4 style={{ color: '#60a5fa', marginTop: 0 }}>IBKR</h4>
                    {[
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', fontWeight: 700, color: 'white', borderTop: '1px solid rgba(59,130,246,0.3)', paddingTop: '0.5rem', marginTop: '0.5rem' }}>
                      <span>SKUPAJ:</span><span style={{ color: '#f87171' }}>-€{calculations.ibkrTotalCosts.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* FIFO */}
              {activeTab === 'fifo' && (
                <div style={{ background: 'rgba(30,41,59,0.3)', border: '1px solid rgba(71,85,105,0.5)', borderRadius: '12px', padding: '1.25rem' }}>
                  <h3 style={{ color: 'white', marginTop: 0 }}>FIFO razčlemba (IBKR)</h3>
                  <p style={{ color: '#94a3b8', fontSize: '0.75rem' }}>Vsak lot ima svoj rok lastništva. Davek se računa per lot.</p>
                  {[
                    { period: '15+ let', rate: '0%', color: '#34d399', bg: 'rgba(16,185,129,0.1)', border: 'rgba(16,185,129,0.3)', key: '15+' },
                    ...
                    { period: '0-5 let', rate: '25%', color: '#f87171', bg: 'rgba(239,68,68,0.1)', border: 'rgba(239,68,68,0.3)', key: '0-5' },
                  ].map(p => (
                    <div key={p.period} style={{ background: p.bg, border: `1px solid ${p.border}`, borderRadius: '8px', padding: '0.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                      <div>
                        ...
                        <p style={{ color: '#64748b', fontSize: '0.65rem', margin: 0 }}>{p.rate} davek</p>
                      </div>
                  ...
                  <div style={{ background: 'rgba(30,41,59,0.5)', borderRadius: '8px', padding: '0.75rem', display: 'flex', justifyContent: 'space-between', marginTop: '0.5rem' }}>
                    <p style={{ color: 'white', fontWeight: 600, margin: 0 }}>SKUPAJ:</p>
                    <p style={{ color: 'white', fontSize: '1.5rem', fontWeight: 800, margin: 0 }}>€{Math.round(calculations.ibkrTracking.slovenianCapitalGainsTax).toLocaleString()}</p>
                  </div>
                  <p style={{ color: '#94a3b8', fontSize: '0.75rem', marginTop: '0.5rem' }}>
                    Efektivna stopnja: <strong style={{ color: '#fbbf24' }}>{calculations.effectiveIbkrTaxRate}%</strong> ({calculations.lotStats.ibkrLots} lotov, avg {calculations.lotStats.avgYears} let)
                  </p>
                </div>
              )}
              ...
              {activeTab === 'transparency' && (
                <div style={{ background: 'rgba(30,41,59,0.3)', border: '1px solid rgba(71,85,105,0.5)', borderRadius: '12px', padding: '1.25rem' }}>
                  <h3 style={{ color: 'white', marginTop: 0 }}>Kronološki izračun</h3>
                  {[
                    { color: '#10b981', title: 'KORAK 1: Začetek', content: `INR vloženo: €${calculations.totalInvestedInr.toLocaleString()} | IBKR vloženo: €${calculations.totalInvestedIbkr.toLocaleString()}` },
                    { color: '#3b82f6', title: 'KORAK 2: Stroški nakupa', content: `INR: SEPA 1,20€ + provizija ${openInr2026 ? '0,15%' : '0,30%'} + 6€ poravnava | IBKR: ~1,25€` },
                    { color: '#a855f7', title: 'KORAK 3: Mesečna rast', content: `Neto donos: ${(etf.historicalReturn - etf.expense).toFixed(2)}%/leto` },
                    { color: '#f59e0b', title: 'KORAK 4: Dividende', content: etf.isAccumulating ? 'Akumulacijski - brez vmesnih davkov' : 'INR: -0,50% payout | IBKR: -25% davek' },
                    { color: '#f97316', title: 'KORAK 5: Letno vzdrževanje INR', content: `max(8,40€, vrednost × 0,0045%) = ~€${Math.round(calculations.inrTracking.ilirikaCustody / yearsToInvest)}/leto` },
                    { color: '#ef4444', title: 'KORAK 6: Končni davki', content: `INR: ${yearsToInvest >= 15 ? '0%' : '15%'} = €${Math.round(calculations.inrTracking.finalTaxAtWithdrawal)} | IBKR FIFO: ${calculations.effectiveIbkrTaxRate}% = €${Math.round(calculations.ibkrTracking.slovenianCapitalGainsTax)}` },
                  ].map(step => (
                    <div key={step.title} style={{ background: 'rgba(15,23,42,0.5)', borderRadius: '8px', padding: '0.75rem', borderLeft: `4px solid ${step.color}`, marginBottom: '0.5rem' }}>
                      <p style={{ color: '#cbd5e1', fontSize: '0.75rem', fontWeight: 600, margin: '0 0 0.25rem' }}>{step.title}</p>
                      <p style={{ color: '#94a3b8', fontSize: '0.75rem', margin: 0 }}>{step.content}</p>
                    </div>
                  ))}
                  <div style={{ background: 'rgba(16,185,129,0.1)', borderRadius: '8px', padding: '0.75rem', borderLeft: '4px solid #34d399' }}>
                    ...
                    <p style={{ color: '#cbd5e1', fontSize: '0.75rem', margin: 0 }}>
                      INR: <strong style={{ color: '#34d399' }}>€{calculations.inrFinalValue.toLocaleString()}</strong> | IBKR: <strong style={{ color: '#60a5fa' }}>€{calculations.ibkrFinalValue.toLocaleString()}</strong>
                    </p>
                  </div>
                </div>
              )}
              ...
              {/* Insights */}
              <div style={{ background: 'rgba(30,41,59,0.5)', border: '1px solid rgba(71,85,105,0.5)', borderRadius: '12px', padding: '1rem' }}>
                <p style={{ color: '#e2e8f0', fontSize: '0.875rem', fontWeight: 600, margin: '0 0 0.5rem' }}>Ugotovitve</p>
                <ul style={{ fontSize: '0.75rem', color: '#94a3b8', paddingLeft: '1rem', margin: 0, lineHeight: 2 }}>
                  {calculations.inrBlocked && <li style={{ color: '#f87171' }}>INR blokiran - prilagodi začetni kapital na max €20.000</li>}
                  {customIbkr && <li style={{ color: '#60a5fa' }}>Ločeni IBKR parametri aktivni</li>}
                  {!etf.isAccumulating && <li style={{ color: '#fbbf24' }}>Distribucijski ETF: INR ima prednost (brez 25% davka na dividende)</li>}
                  {etf.isAccumulating && <li style={{ color: '#93c5fd' }}>Akumulacijski ETF: razlika med INR/IBKR je manjša</li>}
                  {investmentMode === 'dca' && monthlyContribution * 12 > 5000 && <li style={{ color: '#fbbf24' }}>Presežek nad €5k/leto se NE vloži v INR</li>}
                </ul>
              </div>
            </div>
          </div>
          ...
          {/* ETF Library */}
          <div>
            <h2 style={{ color: 'white', fontSize: '1.25rem', fontWeight: 700, marginBottom: '1rem' }}>ETF knjižnica</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem' }}>
              {Object.entries(ETF_PROFILES).map(([key, p]) => (
                <div key={key} onClick={() => { setSelectedEtf(key); setExpandedDetails(expandedDetails === key ? null : key); }}
                  style={{ background: selectedEtf === key ? 'rgba(16,185,129,0.1)' : 'rgba(30,41,59,0.3)', border: `1px solid ${selectedEtf === key ? 'rgba(16,185,129,0.5)' : 'rgba(71,85,105,0.5)'}`, borderRadius: '12px', padding: '0.75rem', cursor: 'pointer', transition: 'all 0.15s' }}>
                  <p style={{ color: 'white', fontWeight: 700, fontSize: '0.875rem', margin: '0 0 0.1rem' }}>{p.name}</p>
                  <p style={{ color: '#64748b', fontSize: '0.65rem', margin: '0 0 0.5rem' }}>{p.fullName}</p>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.25rem' }}>
                    <div><p style={{ color: '#64748b', fontSize: '0.6rem', margin: 0 }}>DIV</p><p style={{ color: '#34d399', fontWeight: 700, fontSize: '0.8rem', margin: 0 }}>{p.grossDividendYield}%</p></div>
                    <div><p style={{ color: '#64748b', fontSize: '0.6rem', margin: 0 }}>DONOS</p><p style={{ color: '#60a5fa', fontWeight: 700, fontSize: '0.8rem', margin: 0 }}>{p.historicalReturn}%</p></div>
                    <div><p style={{ color: '#64748b', fontSize: '0.6rem', margin: 0 }}>TER</p><p style={{ color: '#fbbf24', fontWeight: 700, fontSize: '0.8rem', margin: 0 }}>{p.expense}%</p></div>
                  </div>
                  <p style={{ color: '#64748b', fontSize: '0.65rem', margin: '0.4rem 0 0' }}>{p.distribution} | {p.riskLevel}</p>
                  {expandedDetails === key && (
                    <div style={{ marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid #334155', fontSize: '0.7rem' }}>
                      <p style={{ color: '#94a3b8', margin: '0 0 0.25rem' }}>{p.description}</p>
                      <p style={{ color: '#34d399', margin: '0 0 0.1rem' }}>+ {p.pros.join(', ')}</p>
                      <p style={{ color: '#f87171', margin: 0 }}>- {p.cons.join(', ')}</p>
                    </div>
                  )}
                </div>
              ))}
        ...
        <div style={{ marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px solid #1e293b', fontSize: '0.75rem', color: '#475569' }}>
